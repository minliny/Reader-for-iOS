...
| Demo proof evidence | Reader UI | canonical `frontend-demo-optimized/` coverage、代表截图、selector/motion 映射 | First P0 representative proof ready |
...
| 领域 | 平台必须提供 | 接受标准 |
|---|---|---|
| Build and smoke | 构建日志、启动 smoke、目标平台版本 | App 可安装/启动，入口不是 WebView 加载 `frontend-demo-optimized/` |
...
- 使用 WebView 运行 `frontend-demo-optimized/` 作为正式 UI。
