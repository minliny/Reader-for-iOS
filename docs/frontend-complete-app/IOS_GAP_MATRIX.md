...
| `xcodebuild test -project ReaderForIOS.xcodeproj -scheme ReaderForIOSApp -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.5' CODE_SIGNING_ALLOWED=NO ONLY_ACTIVE_ARCH=YES ARCHS=arm64 -only-testing:ReaderAppTests/ReaderContractAdapterSlice0Tests` | BLOCKED locally. A focused project test attempt timed out after 180 seconds at `Resolve Package Graph` before XCTest execution. |
