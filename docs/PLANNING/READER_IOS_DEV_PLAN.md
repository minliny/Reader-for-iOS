...
| 阶段 | 名称 | 状态 | 预计周期 | 前置条件 |
|------|------|------|----------|----------|
| S0 | 项目状态冻结与边界确认 | ✅ 验收中 | 1-2 天 | 反向拆仓完成 |
| S1 | Reader-Core public API 接入验证 | ⏳ 待开始 | 3-5 天 | S0 完成 |
| S2 | 书源管理真实接入 | ⏳ 待开始 | 2-3 天 | S1 完成 |
| S3 | 搜索流程真实接入 | ⏳ 待开始 | 2-3 天 | S2 完成 |
| S4 | 书籍详情与目录流程真实接入 | ⏳ 待开始 | 2-3 天 | S3 完成 |
| S5 | 阅读页与章节加载真实接入 | ⏳ 待开始 | 3-5 天 | S4 完成 |
| S6 | 书架、阅读进度、章节缓存闭环 | ⏳ 待开始 | 3-5 天 | S5 完成 |
| S7 | 设置、错误处理、状态展示完善 | ⏳ 待开始 | 2-3 天 | S6 完成 |
| S8 | 测试、稳定化与发布前检查 | ⏳ 待开始 | 3-5 天 | S7 完成 |
...
- [x] `scripts/check_ios_boundary.sh` 运行通过 (result=PASS)
- [x] 所有 Target 可正常编译
- [x] 所有测试通过
- [x] 边界规则清晰且执行到位
...
### S1: Reader-Core public API 接入验证

#### 阶段目标
- 接入真实的 Reader-Core Parser/Network
- 实现 Mock ↔ Real Core 切换机制
- 验证核心链路与真实 Core 兼容
...
**推荐**: 完成 S0 验收后，进入 S1  
**第一个任务**: S1.P0 Reader-Core public API 接入现状审计
