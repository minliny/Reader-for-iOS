// run_sample_005_nonjs_smoke.swift
// Usage: swift run --package-path Core Sample005NonJSSmokeRunner -- <repo_root>
...
@main
struct Sample005NonJSSmokeRunner {
    ...
    static func main() throws {
        ...
        let bookSource = try JSONDecoder().decode(
            BookSource.self,
            from: try readData("samples/booksources/p0_non_js/sample_005.json")
        )
        let searchHTML  = try readStr("samples/fixtures/html/sample_005_search.html")
        let tocHTML     = try readStr("samples/fixtures/html/sample_005_toc.html")
        let contentHTML = try readStr("samples/fixtures/html/sample_005_content.html")

        let expSearch  = try JSONDecoder().decode(ExpectedSearchFile.self,  from: try readData("samples/expected/search/sample_005.json"))
        let expToc     = try JSONDecoder().decode(ExpectedTocFile.self,     from: try readData("samples/expected/toc/sample_005.json"))
        let expContent = try JSONDecoder().decode(ExpectedContentFile.self, from: try readData("samples/expected/content/sample_005.json"))
        ...
        out.append("  notes: \"sample_005 full chain smoke: search -> toc -> content (tag selectors)\"")

        let yaml = out.joined(separator: "\n") + "\n"

        let reportPath = rp("samples/reports/latest/sample_005_nonjs_smoke_result.yml")
