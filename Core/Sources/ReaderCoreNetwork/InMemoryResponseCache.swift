...
public actor InMemoryResponseCache: ResponseCache {
    ...
    public func get(key: ResponseCacheKey, now: Date) async -> CachedHTTPResponse? {
    ...
    public func put(response: CachedHTTPResponse, key: ResponseCacheKey) async {
        storage[key] = response
    }

    /// Removes the entry for the given key. No-op if key is not present.
    public func remove(key: ResponseCacheKey) async {
        storage.removeValue(forKey: key)
    }
