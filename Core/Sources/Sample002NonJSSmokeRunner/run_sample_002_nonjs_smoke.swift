// run_sample_002_nonjs_smoke.swift
// Usage: swift run --package-path Core Sample002NonJSSmokeRunner -- <repo_root>
...
@main
struct Sample002NonJSSmokeRunner {
    ...
    static func main() throws {
        ...
        let bookSource = try JSONDecoder().decode(
            BookSource.self,
            from: try readData("samples/booksources/p0_non_js/sample_002.json")
        )
        let searchHTML  = try readStr("samples/fixtures/html/sample_002_search.html")
        let tocHTML     = try readStr("samples/fixtures/html/sample_002_toc.html")
        let contentHTML = try readStr("samples/fixtures/html/sample_002_content.html")

        let expSearch  = try JSONDecoder().decode(ExpectedSearchFile.self,  from: try readData("samples/expected/search/sample_002.json"))
        let expToc     = try JSONDecoder().decode(ExpectedTocFile.self,     from: try readData("samples/expected/toc/sample_002.json"))
        let expContent = try JSONDecoder().decode(ExpectedContentFile.self, from: try readData("samples/expected/content/sample_002.json"))
        ...
        do {
            actualToc = try engine.parseTOCResponse(
                Data(tocHTML.utf8),
                source: bookSource,
                detailURL: "http://fixture2.local/book/1.html"
            )
        ...
        do {
            let page = try engine.parseContentResponse(
                ...
                source: bookSource,
                chapterURL: "http://fixture2.local/chapter/1.html"
            )
        ...
        out.append("  notes: \"sample_002 full chain smoke: search -> toc -> content\"")

        let yaml = out.joined(separator: "\n") + "\n"

        let reportPath = rp("samples/reports/latest/sample_002_nonjs_smoke_result.yml")
