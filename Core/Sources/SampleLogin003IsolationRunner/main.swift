// SampleLogin003IsolationRunner/main.swift
// Minimum-variable isolation for sample_login_003 (the-internet.herokuapp.com)
// Usage: swift run --package-path Core SampleLogin003IsolationRunner -- <repo_root>
...
private struct Analysis { let responseClass: String; let setCookie: Bool; let login: Bool; let js: Bool; let found: Bool; let successMarkersSeen: [String] }
private struct StepRecord {
    let stepId: String; let changedVar: String; let policy: [String: Bool]
    let httpStatus: Int?; let contentType: String?; let finalURL: String; let responseClass: String
    let setCookie: Bool; let login: Bool; let js: Bool; let found: Bool
    let candidateFailureType: String; let secondaryReason: String; let decision: String; let notes: String
}
private struct DecisionSummary { let finalFT: String; let finalSR: String; let winningStep: String?; let actualLevel: String; let accessTier: String; let matrixUpdate: Bool; let notes: String }
...
private func stringValue(_ value: JSONValue?) -> String? { guard case .string(let s)? = value else { return nil }; return s }
...
private func boolValue(_ value: JSONValue?) -> Bool? { guard case .bool(let b)? = value else { return nil }; return b }
private func loadLoginFlow(from source: BookSource) -> LoginFlowConfig? {
    ...
    return LoginFlowConfig(method: method, contentType: contentType, actionURL: actionURL, usernameField: usernameField, passwordField: passwordField, usernameValue: usernameValue, passwordValue: passwordValue, successURL: successURL, successMarkers: successMarkers, failureMarkers: failureMarkers)
}
...
private func analyze(_ response: HTTPResponse, loginFlow: LoginFlowConfig) -> Analysis {
    ...
    let found = response.statusCode == 200 && loginFlow.successMarkers.allSatisfy { body.contains($0) }
    let setCookie = response.headers.keys.contains { $0.lowercased() == "set-cookie" }
    return Analysis(responseClass: responseClass(for: response.statusCode), setCookie: setCookie, login: login, js: js, found: found, successMarkersSeen: successMarkersSeen)
}
...
private func makeRecord(stepId: String, changedVar: String, policy: [String: Bool], response: HTTPResponse?, finalURL: String, loginFlow: LoginFlowConfig) -> StepRecord {
    let analysis = response.map { analyze($0, loginFlow: loginFlow) } ?? Analysis(responseClass: "network_error", setCookie: false, login: false, js: false, found: false, successMarkersSeen: [])
    let candidateFailureType: String
    let secondaryReason: String
    switch stepId {
    case "TEST-001": candidateFailureType = "NETWORK_POLICY_MISMATCH"; secondaryReason = analysis.found ? "missing_header" : "baseline_controlled_fetch_failed"
    case "TEST-002": candidateFailureType = "NETWORK_POLICY_MISMATCH"; secondaryReason = analysis.found ? "missing_referer" : "baseline_controlled_fetch_failed"
    case "TEST-003": candidateFailureType = analysis.found ? "COOKIE_REQUIRED" : "NETWORK_POLICY_MISMATCH"; secondaryReason = analysis.found ? "missing_required_cookie" : "baseline_controlled_fetch_failed"
    case "TEST-004": candidateFailureType = "NETWORK_POLICY_MISMATCH"; secondaryReason = analysis.found ? "undetermined_after_isolation" : "baseline_controlled_fetch_failed"
    case "TEST-005": candidateFailureType = "NETWORK_POLICY_MISMATCH"; secondaryReason = analysis.found ? "redirect_not_handled" : "baseline_controlled_fetch_failed"
    case "TEST-006": candidateFailureType = analysis.found ? "LOGIN_REQUIRED" : "NETWORK_POLICY_MISMATCH"; secondaryReason = analysis.found ? "login_state_absent" : "baseline_controlled_fetch_failed"
    case "TEST-007": candidateFailureType = "NETWORK_POLICY_MISMATCH"; secondaryReason = analysis.js ? "js_challenge_observed" : "baseline_controlled_fetch_failed"
    default: candidateFailureType = "NETWORK_POLICY_MISMATCH"; secondaryReason = "baseline_controlled_fetch_failed"
    }
    let decision: String = analysis.found ? "match_found:\(candidateFailureType)" : (stepId == "TEST-007" ? "stop_undetermined" : "continue_isolation")
    let notes: String
    if analysis.found { notes = "Success markers seen: \(analysis.successMarkersSeen.joined(separator: ", "))" }
    else if analysis.login { notes = "Login markers observed and secure markers absent." }
    else if analysis.js { notes = "JS challenge markers observed." }
    else if response == nil { notes = "Network request failed before HTTP response." }
    else { notes = "Secure markers absent." }
    return StepRecord(stepId: stepId, changedVar: changedVar, policy: policy, httpStatus: response?.statusCode, contentType: response?.headers["Content-Type"] ?? response?.headers["content-type"], finalURL: finalURL, responseClass: analysis.responseClass, setCookie: analysis.setCookie, login: analysis.login, js: analysis.js, found: analysis.found, candidateFailureType: candidateFailureType, secondaryReason: secondaryReason, decision: decision, notes: notes)
}
private func applyDecisionRules(_ records: [StepRecord]) -> DecisionSummary {
    ...
    if winners.isEmpty {
        ...
        return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: jsObserved ? "js_challenge_observed" : "undetermined_after_isolation", winningStep: nil, actualLevel: "D", accessTier: jsObserved ? "C" : "B3", matrixUpdate: true, notes: jsObserved ? "All isolation steps failed and JS challenge markers were observed." : "All isolation steps failed without secure-page success markers.")
    }
    if winners.count > 1 {
        return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "undetermined_after_isolation", winningStep: winners.first?.stepId, actualLevel: "D", accessTier: "B3", matrixUpdate: false, notes: "Multiple isolation steps reached success markers: \(winners.map(\.stepId).joined(separator: ", ")).")
    }
    let winner = winners[0]
    switch winner.stepId {
    case "TEST-001": return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "missing_header", winningStep: winner.stepId, actualLevel: "B", accessTier: "B1", matrixUpdate: true, notes: "User-Agent alone unlocked the secure page.")
    case "TEST-002": return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "missing_referer", winningStep: winner.stepId, actualLevel: "B", accessTier: "B1", matrixUpdate: true, notes: "Referer alone unlocked the secure page.")
    case "TEST-003": return DecisionSummary(finalFT: "COOKIE_REQUIRED", finalSR: "missing_required_cookie", winningStep: winner.stepId, actualLevel: "B", accessTier: "B2", matrixUpdate: true, notes: "Cookie jar alone unlocked the secure page.")
    case "TEST-004": return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "undetermined_after_isolation", winningStep: winner.stepId, actualLevel: "D", accessTier: "B3", matrixUpdate: false, notes: "Retry changed the outcome, so login gating is unconfirmed.")
    case "TEST-005": return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "redirect_not_handled", winningStep: winner.stepId, actualLevel: "B", accessTier: "B2", matrixUpdate: true, notes: "Redirect behavior, not login state, changed the result.")
    case "TEST-006": return DecisionSummary(finalFT: "LOGIN_REQUIRED", finalSR: "login_state_absent", winningStep: winner.stepId, actualLevel: "A", accessTier: "B3", matrixUpdate: true, notes: "Baseline stayed outside the secure markers and only the real form-login step reached the secure page.")
    case "TEST-007": return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "js_challenge_observed", winningStep: winner.stepId, actualLevel: "D", accessTier: "C", matrixUpdate: true, notes: "JS challenge markers dominate the outcome.")
    default: return DecisionSummary(finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "undetermined_after_isolation", winningStep: nil, actualLevel: "D", accessTier: "B3", matrixUpdate: false, notes: "Unknown winning step.")
    }
}
...
guard let repoRoot = positional.first else { fputs("Usage: SampleLogin003IsolationRunner -- <repo_root>\n", stderr); exit(1) }
...
Task {
    ...
    do {
        ...
        guard let loginFlow = loadLoginFlow(from: source) else { throw NSError(domain: "SampleLogin003IsolationRunner", code: 1, userInfo: [NSLocalizedDescriptionKey: "xReaderLoginFlow missing or invalid"]) }
        ...
        let reportPrefix = "isolation_login_003_\(Int(Date().timeIntervalSince1970))"
        ...
        for _ in 0..<3 {
            ...
            if let retryResponse, analyze(retryResponse, loginFlow: loginFlow).found { break }
        }
        ...
        stepLines.append("sampleId: \"sample_login_003\"")
        stepLines.append("")
        stepLines.append("records:")
        for record in records { appendRecord(record, to: &stepLines) }
        try (stepLines.joined(separator: "\n") + "\n").write(toFile: rp("samples/reports/latest/fetch_isolation_step_records_sample_login_003.yml"), atomically: true, encoding: .utf8)

        let summaryLines = [
            "reportId: \(ys(reportPrefix + "_summary"))",
            "generatedAt: \(ys(generatedAt))",
            "phase: \"p1_login\"",
            "sampleId: \"sample_login_003\"",
            "",
            "decision:",
            "  baselineFailureType: \"NETWORK_POLICY_MISMATCH\"",
            "  finalPrimaryFailureType: \(ys(decision.finalFT))",
            "  finalSecondaryReason: \(ys(decision.finalSR))",
            "  winningStep: \(decision.winningStep.map { ys($0) } ?? "null")",
            "  actualLevel: \(ys(decision.actualLevel))",
            "  accessTier: \(ys(decision.accessTier))",
            "  matrixUpdate:",
            "    update: \(yb(decision.matrixUpdate))",
            "    failureType: \(ys(decision.finalFT))",
            "    secondaryReason: \(ys(decision.finalSR))",
            "  notes: \(ys(decision.notes))"
        ]
        try (summaryLines.joined(separator: "\n") + "\n").write(toFile: rp("samples/reports/latest/fetch_isolation_decision_summary_sample_login_003.yml"), atomically: true, encoding: .utf8)
        print("step_records: \(rp("samples/reports/latest/fetch_isolation_step_records_sample_login_003.yml"))")
        print("decision_summary: \(rp("samples/reports/latest/fetch_isolation_decision_summary_sample_login_003.yml"))")
