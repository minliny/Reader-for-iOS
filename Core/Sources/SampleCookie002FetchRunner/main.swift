// SampleCookie002FetchRunner/main.swift
// Baseline-only fetch for sample_cookie_002 (qidian.com)
// Usage: swift run --package-path Core SampleCookie002FetchRunner -- <repo_root>
...
) -> (setCookie: Bool, login: Bool, js: Bool, found: Bool) {
...
guard let repoRoot = positional.first else {
    fputs("Usage: SampleCookie002FetchRunner -- <repo_root>\n", stderr)
    exit(1)
}
    ...
    do {
        ...
        do {
            ...
            if searchResultMarkerObserved {
                ...
                notes = "Baseline fetch succeeded without cookies; isolation not required"
            } else {
                fetchStatus = "failed"
        ...
        let runId = "fetch_cookie_002_\(Int(Date().timeIntervalSince1970))"
        var out: [String] = []
        out.append("reportId: \(ys(runId))")
        out.append("generatedAt: \(ys(now))")
        out.append("phase: \"p1_cookie\"")
        out.append("sampleId: \"sample_cookie_002\"")
        out.append("")
        out.append("baseline:")
        ...
        let path = rp("samples/reports/latest/fetch_result_sample_cookie_002.yml")
        try yaml.write(toFile: path, atomically: true, encoding: .utf8)
        print("report: \(path)")
        print("fetchStatus: \(fetchStatus)")
        print("primaryFailureType: NETWORK_POLICY_MISMATCH")
