// SampleCookie002IsolationRunner/main.swift
// Minimum-variable isolation for sample_cookie_002 (qidian.com)
// Usage: swift run --package-path Core SampleCookie002IsolationRunner -- <repo_root>
...
private func applyDecisionRules(_ records: [StepRecord]) -> DecisionSummary {
    ...
    if winners.isEmpty {
        ...
        return DecisionSummary(
            ...
            winningStep: nil,
            actualLevel: jsObs ? "D" : "D",
            accessTier: jsObs ? "C" : "B2",
            matrixUpdate: true,
            notes: jsObs
                ? "JS challenge observed across all 8 steps; no step resolved access. Site classified tier C."
                : "All 8 isolation steps failed; no JS challenge observed. Root cause undetermined."
        )
    }
    if winners.count > 1 {
        return DecisionSummary(
            ...
            accessTier: "B2", matrixUpdate: false,
    ...
    case "TEST-004":
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"undetermined_after_isolation",
            winningStep:w.stepId, actualLevel:"D", accessTier:"B2", matrixUpdate:false,
    ...
    case "TEST-006":
        ...
        return DecisionSummary(finalFT:"LOGIN_REQUIRED", finalSR:sr,
            ...
            notes:"Login step resolved access (\(sr)). Site is tier B3.")
    case "TEST-007":
    ...
    default:
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"undetermined_after_isolation",
            winningStep:nil, actualLevel:"D", accessTier:"B2", matrixUpdate:false,
...
guard let repoRoot = positional.first else {
    fputs("Usage: SampleCookie002IsolationRunner -- <repo_root>\n", stderr)
    exit(1)
}
    ...
    do {
        let bs = try JSONDecoder().decode(
            ...
            from: try Data(contentsOf: URL(fileURLWithPath:
                rp("samples/booksources/p1_cookie/sample_cookie_002.json")))
        )
        ...
        let homeURL    = bs.bookSourceUrl ?? "https://www.qidian.com"
        ...
        stepsOut.append("sampleId: \"sample_cookie_002\"")
        stepsOut.append("")
        stepsOut.append("records:")
        for r in records { appendRecord(r, to: &stepsOut) }
        let stepsYAML = stepsOut.joined(separator: "\n") + "\n"
        let stepsPath = rp("samples/reports/latest/fetch_isolation_step_records_sample_cookie_002.yml")
        ...
        sumOut.append("sampleId: \"sample_cookie_002\"")
        sumOut.append("")
        ...
        let sumPath = rp("samples/reports/latest/fetch_isolation_decision_summary_sample_cookie_002.yml")
