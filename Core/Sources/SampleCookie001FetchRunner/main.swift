// SampleCookie001FetchRunner/main.swift
// Baseline-only fetch for sample_cookie_001
// Usage: swift run --package-path Core SampleCookie001FetchRunner -- <repo_root>
...
private func analyzeBody(
    ...
    // setCookie is passed in separately from response headers
...
guard let repoRoot = positional.first else {
    fputs("Usage: SampleCookie001FetchRunner -- <repo_root>\n", stderr)
    exit(1)
}
...
Task {
    ...
    do {
        ...
        let runId = "fetch_cookie_001_\(Int(Date().timeIntervalSince1970))"
        var out: [String] = []
        out.append("reportId: \(ys(runId))")
        out.append("generatedAt: \(ys(now))")
        out.append("phase: \"p1_cookie\"")
        out.append("sampleId: \"sample_cookie_001\"")
        out.append("")
        out.append("baseline:")
        ...
        let path = rp("samples/reports/latest/fetch_result_sample_cookie_001.yml")
