// run_sample_001_nonjs_smoke.swift
// Usage: swift run Sample001NonJSSmokeRunner --package-path Core -- <repo_root>
// Requires: add executableTarget("Sample001NonJSSmokeRunner") in Core/Package.swift
//           with this file as its Sources entry.

import Foundation
import ReaderCoreModels
import ReaderCoreParser

// MARK: - Expected file schemas
...
@main
struct Sample001NonJSSmokeRunner {
    ...
    static func main() throws {
        ...
        let bookSource = try JSONDecoder().decode(
            BookSource.self,
            from: try readData("samples/booksources/p0_non_js/sample_001.json")
        )
        let searchHTML  = try readStr("samples/fixtures/html/sample_001_search.html")
        let tocHTML     = try readStr("samples/fixtures/html/sample_001_toc.html")
        let contentHTML = try readStr("samples/fixtures/html/sample_001_content.html")

        let expSearch  = try JSONDecoder().decode(ExpectedSearchFile.self,  from: try readData("samples/expected/search/sample_001.json"))
        let expToc     = try JSONDecoder().decode(ExpectedTocFile.self,     from: try readData("samples/expected/toc/sample_001.json"))
        let expContent = try JSONDecoder().decode(ExpectedContentFile.self, from: try readData("samples/expected/content/sample_001.json"))
        ...
        out.append("  notes: \"sample_001 full chain smoke: search -> toc -> content\"")

        let yaml = out.joined(separator: "\n") + "\n"

        // ── Write report ──────────────────────────────────────────────────
        let reportPath = rp("samples/reports/latest/sample_001_nonjs_smoke_result.yml")
