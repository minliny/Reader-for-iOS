// SampleLogin001IsolationRunner/main.swift
// Minimum-variable isolation for sample_login_001 (biquge.com.cn)
// Usage: swift run --package-path Core SampleLogin001IsolationRunner -- <repo_root>
...
private struct DecisionSummary {
    ...
    let accessTier: String; let matrixUpdate: Bool; let notes: String
}
private func applyDecisionRules(_ records: [StepRecord]) -> DecisionSummary {
    ...
    if winners.isEmpty {
        ...
        let loginObs = records.first { $0.login } != nil
        if jsObs {
            return DecisionSummary(
                finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "js_challenge_observed",
                winningStep: nil, actualLevel: "D", accessTier: "C", matrixUpdate: true,
                notes: "JS challenge observed across all steps. Site classified tier C."
            )
        }
        if loginObs {
            return DecisionSummary(
                finalFT: "LOGIN_REQUIRED", finalSR: "login_marker_observed_no_step_resolved",
                winningStep: nil, actualLevel: "D", accessTier: "B3", matrixUpdate: true,
                notes: "Login marker observed but no isolation step resolved search access. Manual login credential required."
            )
        }
        return DecisionSummary(
            finalFT: "NETWORK_POLICY_MISMATCH", finalSR: "undetermined_after_isolation",
            winningStep: nil, actualLevel: "D", accessTier: "B3", matrixUpdate: true,
            notes: "All 8 isolation steps failed; no JS challenge, no login marker. Root cause undetermined."
        )
    }
    if winners.count > 1 {
        return DecisionSummary(
            ...
            accessTier: "B3", matrixUpdate: false,
            notes: "Multiple steps succeeded: \(winners.map{$0.stepId}.joined(separator:", "))"
        )
    }
    let w = winners[0]
    switch w.stepId {
    case "TEST-001":
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"missing_header",
            winningStep:w.stepId, actualLevel:"B", accessTier:"B1", matrixUpdate:true,
            notes:"User-Agent header resolved access. Site is tier B1.")
    case "TEST-002":
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"missing_referer",
            winningStep:w.stepId, actualLevel:"B", accessTier:"B1", matrixUpdate:true,
            notes:"Referer header resolved access. Site is tier B1.")
    case "TEST-003":
        let sr = w.setCookie ? "missing_required_cookie" : "cookie_expired"
        return DecisionSummary(finalFT:"COOKIE_REQUIRED", finalSR:sr,
            winningStep:w.stepId, actualLevel:"B", accessTier:"B2", matrixUpdate:true,
            notes:"Cookie jar resolved access (\(sr)). Site confirmed tier B2.")
    case "TEST-004":
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"undetermined_after_isolation",
            winningStep:w.stepId, actualLevel:"D", accessTier:"B3", matrixUpdate:false,
            notes:"Retry resolved access; transient failure, root cause undetermined.")
    case "TEST-005":
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"redirect_not_handled",
            winningStep:w.stepId, actualLevel:"B", accessTier:"B2", matrixUpdate:true,
            notes:"Redirect handling resolved access. Site is tier B2.")
    case "TEST-006":
        let sr = w.login ? "auth_redirect_detected" : "login_state_absent"
        return DecisionSummary(finalFT:"LOGIN_REQUIRED", finalSR:sr,
            winningStep:w.stepId, actualLevel:"B", accessTier:"B3", matrixUpdate:true,
            notes:"Login step resolved access (\(sr)). Site confirmed tier B3.")
    case "TEST-007":
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"js_challenge_observed",
            winningStep:w.stepId, actualLevel:"D", accessTier:"C", matrixUpdate:true,
            notes:"JS challenge observed in TEST-007. Site classified tier C.")
    default:
        return DecisionSummary(finalFT:"NETWORK_POLICY_MISMATCH", finalSR:"undetermined_after_isolation",
            winningStep:nil, actualLevel:"D", accessTier:"B3", matrixUpdate:false,
            notes:"Unknown winning step")
    }
}
...
guard let repoRoot = positional.first else {
    fputs("Usage: SampleLogin001IsolationRunner -- <repo_root>\n", stderr)
    exit(1)
}
...
Task {
    ...
    do {
        ...
        let homeURL    = bs.bookSourceUrl ?? "https://www.biquge.com.cn"
        ...
        let runId = "isolation_login_001_\(Int(Date().timeIntervalSince1970))"
        ...
        let blSet = blR.flatMap { r in r.headers.keys.contains { $0.lowercased()=="set-cookie" } ? true : Optional(false) } ?? false
        records.append(makeRecord(
            ...
            analysis:blA, url:searchURL, ct:blR?.headers["Content-Type"] ?? blR?.headers["content-type"],
            httpStatus:blR?.statusCode, setCookieHeader:blSet
        ))
        ...
        _ = await send(homeURL, client:c3, useJar:true)
        ...
        // ── TEST-006: login (GET loginUrl, collect cookies, then search) ──
        ...
        if !loginURL.isEmpty {
            ...
            _ = await send(loginURL, client:c6, useJar:true)
        ...
        stepsOut.append("phase: \"p1_login\"")
        stepsOut.append("sampleId: \"sample_login_001\"")
        stepsOut.append("")
        stepsOut.append("records:")
        for r in records { appendRecord(r, to: &stepsOut) }
        let stepsYAML = stepsOut.joined(separator: "\n") + "\n"
        let stepsPath = rp("samples/reports/latest/fetch_isolation_step_records_sample_login_001.yml")
        ...
        sumOut.append("phase: \"p1_login\"")
        sumOut.append("sampleId: \"sample_login_001\"")
        sumOut.append("")
        ...
        sumOut.append("  accessTier: \(ys(ds.accessTier))")
        ...
        let sumPath = rp("samples/reports/latest/fetch_isolation_decision_summary_sample_login_001.yml")
        ...
        print("accessTier: \(ds.accessTier)")
    } catch {
        fputs("Fatal: \(error)\n", stderr)
        exitCode = 1
    }
