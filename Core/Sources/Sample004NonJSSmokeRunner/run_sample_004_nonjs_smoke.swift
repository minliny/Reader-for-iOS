// run_sample_004_nonjs_smoke.swift
// Usage: swift run --package-path Core Sample004NonJSSmokeRunner -- <repo_root>
...
@main
struct Sample004NonJSSmokeRunner {
    ...
    static func main() throws {
        ...
        let bookSource = try JSONDecoder().decode(
            BookSource.self,
            from: try readData("samples/booksources/p0_non_js/sample_004.json")
        )
        let searchHTML  = try readStr("samples/fixtures/html/sample_004_search.html")
        let tocHTML     = try readStr("samples/fixtures/html/sample_004_toc.html")
        let contentHTML = try readStr("samples/fixtures/html/sample_004_content.html")

        let expSearch  = try JSONDecoder().decode(ExpectedSearchFile.self,  from: try readData("samples/expected/search/sample_004.json"))
        let expToc     = try JSONDecoder().decode(ExpectedTocFile.self,     from: try readData("samples/expected/toc/sample_004.json"))
        let expContent = try JSONDecoder().decode(ExpectedContentFile.self, from: try readData("samples/expected/content/sample_004.json"))
        ...
        out.append("  notes: \"sample_004 full chain smoke: search -> toc -> content (class+id selectors)\"")

        let yaml = out.joined(separator: "\n") + "\n"

        let reportPath = rp("samples/reports/latest/sample_004_nonjs_smoke_result.yml")
