...
public enum ReaderMotionAdapter {
    ...
    public static func duration(for contractId: ReaderUIContract.MotionId, motion: MotionEnvironment = .shared) -> TimeInterval {
        guard let spec = spec(for: contractId) else { return motion.duration(ReaderMotion.Duration.base) }
        ...
        // - zeroDuration：instant（0）。对应大多数 motion（86/96 条）。
        // - keepDirectManipulation：手势驱动链路（drag + snap，10 条），reduced motion
