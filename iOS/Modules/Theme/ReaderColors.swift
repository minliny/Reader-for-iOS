...
/// Reader 颜色 token（light mode 对齐 demo `frontend-demo-optimized/tokens.css` 真源）。
///
...
/// 并最终删除此 enum。
///
/// **状态**：已对齐 demo `--fd-ds-color-*` token 真值。新代码请直接使用
...
public enum ReaderColors {
    ...
    /// `--fd-ds-color-floating-control-bg-alt` #eae1da（= `ReaderDesignTokens.Color.floatingControlBgAlt`）
    ...
    /// demo 无 quickButtonBg token，对齐 `--fd-ds-color-surface-soft` rgba(255,252,248,0.72)（= `ReaderDesignTokens.Color.controlBackground`）
    ...
    /// demo `--fd-ds-color-border` #c1c7cd，半透明用于控件边框。
    ...
    /// demo 无 mutedTrack token，用 `--fd-ds-color-muted` #756f69 半透明。
    ...
    /// `--fd-ds-color-meta-bg` #f5ece6（= `ReaderDesignTokens.Color.metaBg`）
